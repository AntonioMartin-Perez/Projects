...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random() <= epsilon:
        ...
        index = np.argmax(theta @ state_vector)
        (action_index, object_index) = index2tuple(index)

        # action_index = int(index / np.shape(theta @ state_vector)[3])
        # object_index = index - action_index * np.shape(theta @ state_vector)[3]
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    # if terminal:
    #     theta[tuple2index(action_index, object_index)] += ALPHA * (reward - theta @ current_state_vector[tuple2index(action_index, object_index)]) * current_state_vector[[tuple2index(action_index, object_index)]]
    # else:        
    #     theta[tuple2index(action_index, object_index)] += ALPHA * (reward + GAMMA * np.max(theta @ next_state_vector) - theta @ current_state_vector[tuple2index(action_index, object_index)]) * current_state_vector[tuple2index(action_index, object_index)]
